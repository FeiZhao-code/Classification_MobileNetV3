...
def main():
    ...
    origin_image_path = os.path.join("image/original")
    assert os.path.exists(origin_image_path), "path '{}' does not exist.".format(origin_image_path)

    image_class = [cla for cla in os.listdir(origin_image_path)
                    if os.path.isdir(os.path.join(origin_image_path, cla))]

    # 建立保存训练集的文件夹
    train_root = os.path.join("image/train")
    mk_file(train_root)
    ...
    # 建立保存验证集的文件夹
    val_root = os.path.join("image/val")
    mk_file(val_root)
    for cla in image_class:
        # 建立每个类别对应的文件夹
